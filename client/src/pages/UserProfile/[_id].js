            ...
            <div className={style.userProfileContainer}>
              <div>
                <img
                  ...
                  src={getUserProfileData?.profilePicture}
                />
              </div>
              <div className={style.userInfoContainer}>
                ...
                <div className={style.followBtn}>
                  ...
                  >
                    {getUserProfileData?.followers?.length} followers
                  </button>
                  <button
                    onClick={() => {
                      setFollowWindow(true);
                  ...
                  >
                    {getUserProfileData?.following?.length} following
                  </button>
                </div>
                <div>
                  ...
                  &nbsp;
                  <span>
                    Joined <JoinedDays date={getUserProfileData?.createdAt} />{" "}
                    days ago
                  </span>
                </div>
              </div>
            </div>
            {ownProfileIsChecking ? (
              <></>
            ) : (
              <div className={style.btnContainer}>
                <button
                  onClick={async () => {
                    await dispatch(
                      FollowMethodAction(getUserProfileData?.userId),
                    );
            ...
            >
              <div style={{ position: "relative" }}>
                ...
                <div className={style.followPersonContainer}>
                  {followersWindow ? (
                    getFollowerListData?.length > 0 ? (
                      getFollowerListData?.map((item) => (
                        <div
                          onClick={() => {
                          ...
                          className={style.followersListStyle}
                          key={item.userId}
                          style={{
                            display: "flex",
                            gap: "10px",
                  ...
                  ) : getFollowingListData?.length > 0 ? (
                    getFollowingListData?.map((item) => (
                      <div
                        onClick={() => {
                          router.push(`/UserProfile/${item?.userId}`);
                          setFollowWindow(false);
